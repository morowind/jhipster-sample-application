...
describe('Location e2e test', () => {
  ...
  describe('new Location page', () => {
    ...
    it('should create an instance of Location', () => {
      cy.get(`[data-cy="streetAddress"]`).type('before airbag bus');
      cy.get(`[data-cy="streetAddress"]`).should('have.value', 'before airbag bus');

      cy.get(`[data-cy="postalCode"]`).type('between');
      cy.get(`[data-cy="postalCode"]`).should('have.value', 'between');

      cy.get(`[data-cy="city"]`).type('衡头市');
      cy.get(`[data-cy="city"]`).should('have.value', '衡头市');

      cy.get(`[data-cy="stateProvince"]`).type('大约 gee ew');
      cy.get(`[data-cy="stateProvince"]`).should('have.value', '大约 gee ew');
